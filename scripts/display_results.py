...
def aggregated_metrics(dir, file_name, save_dir):
    ...
    # relative_metrics_data = ((metrics_data / aggregated_data['PGD Accuracy']['16/255'].to_numpy().reshape(-1, 1)) * 100).astype(int)
...
def benchmarks(dir, file_name, save_dir):
    ...
    g.axes[0].legend()
    for y_index in range(g.axes.shape[0]):
        g.axes[y_index].set_ylabel("Accuracy")
